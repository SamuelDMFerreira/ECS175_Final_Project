...
class OBJLoader {
    ...
    load( gl ) {
        ...
        for (let line of contents.split('\n')){
            ...
            if(line.split(' ')[0] !=  'vt' && line.split(' ')[0] !=  'vn' && line.split(' ')[0] !=  'v' && line.split(' ')[0] !=  'f')
                console.log(line)
            switch(token) {
                case 'mtllib':
        ...
        if (material.hasTexture()) {
            ...
            // Fairly straightforward and similar implementation to Assignment 2
            for (let i = 0; i < vertex_positions.length; i += 3) {
            
                // Push the position data first
                vertex_data.push(vertex_positions[i], vertex_positions[i + 1], vertex_positions[i + 2])
    
                // After we've populated vertex_data with the positions, we stack the normals as well
                vertex_data.push(vertex_normals[i], vertex_normals[i + 1], vertex_normals[i + 2])

                // I think pushing the texture tangents is the same?
                vertex_data.push(vertex_tangents[i], vertex_tangents[i + 1], vertex_tangents[i + 2])
                
                // Since texture coordinates are stored as two-component coordinates instead of 3 (U,V) we need to change how we push this
                // Since the for-loop goes through every 3. We can just divide the index by 3 to go back to iterating every element. 
                // Then we multiply by 2 since we want to go through every two
                vertex_data.push(vertex_texture_coords[i / 3 * 2], vertex_texture_coords[(i / 3  * 2) + 1])
            }
        }
