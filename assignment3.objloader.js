...
class OBJLoader {
    ...
    load( gl ) {
        ...
        for (let i = 0; i < vertex_positions.length; i += 3) {
            vertex_data.push(
                vertex_positions[i], vertex_positions[i+1], vertex_positions[i+2],
                vertex_normals[i], vertex_normals[i+1], vertex_normals[i+2]
            );
            if (material.hasTexture()) {
                
                vertex_data.push(
                    vertex_tangents[i], vertex_tangents[i+1], vertex_tangents[i+2],
                    vertex_texture_coords[(i/3)*2], vertex_texture_coords[(i/3)*2+1]
